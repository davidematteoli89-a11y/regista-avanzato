...
const ID = "the_stats_api" as const;
...
function withControlledError<T>(result: ProviderResult<T>, endpoint: StableProviderEndpoint, config: StableProviderConfig): ProviderResult<T> {
  const reason = !config.configuredInCatalog
    ...
    : !config.active
      ? "TheStatsAPI è disattivato o non configurato: usato il mock."
      : "Adapter TheStatsAPI predisposto ma trasporto HTTP non implementato: usato il mock.";
  return {
    ...result,
...
export class TheStatsApiProvider implements FootballDataProvider {
  readonly id = ID;
...
export const theStatsApiProvider = new TheStatsApiProvider();
