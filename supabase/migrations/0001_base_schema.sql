-- Regista Avanzato — migrazione 0001: schema base staging
-- Non contiene seed, credenziali o policy permissive. Da eseguire una sola
-- volta tramite il runner delle migrazioni su un database staging vuoto.
...
create type public.app_role as enum (
  'free_user', 'editor', 'admin', 'super_admin'
);
...
create table public.data_providers (
  ...
  constraint data_providers_budget_order check (
    ...
    and (monthly_budget_eur is null or warning_budget_eur is null or warning_budget_eur <= monthly_budget_eur)
    and (monthly_budget_eur is null or hard_stop_budget_eur is null or hard_stop_budget_eur <= monthly_budget_eur)
  )
);
...
create table public.team_match_stats (
  ...
  match_id uuid not null references public.matches(id) on delete cascade,
  team_id uuid not null references public.teams(id) on delete cascade,
  source_provider_id uuid not null references public.data_providers(id) on delete restrict,
...
create table public.apify_budget_status (
  ...
  remaining_budget_eur numeric(12, 4) generated always as (
    greatest(0::numeric, budget_limit_eur - estimated_spend_eur)
  ) stored,
  status public.budget_status generated always as (
    case
      when estimated_spend_eur >= hard_stop_eur then 'hard_stop'::public.budget_status
      when estimated_spend_eur >= warning_budget_eur then 'warning'::public.budget_status
...
create table public.news_archive (
  ...
  priority text,
  review_status text,
  internal_warnings text[] not null default '{}'::text[],
...
-- Snapshot persistente dell'Historical Echo. Score, warning e fonti restano
-- colonne operative e non saranno inclusi nelle view pubbliche.
create table public.historical_echoes (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  title text not null,
  summary text,
  ...
  explanation text,
  related_story_id uuid not null references public.story_library(id) on delete restrict,
  modern_match_id uuid references public.matches(id) on delete set null,
...
-- Snapshot interni: nessun auto-publish e nessuna esposizione anon diretta.
create table public.daily_radar_runs (
  id uuid primary key default gen_random_uuid(),
  radar_date date not null unique,
  title text not null,
  run_mode text not null check (run_mode in ('mock', 'dry_run', 'real_disabled')),
  signals jsonb not null default '[]'::jsonb,
  sources jsonb not null default '[]'::jsonb,
  candidates jsonb not null default '[]'::jsonb,
  digest_preview jsonb not null default '{}'::jsonb,
...
create table public.weekly_digests (
  ...
  sources jsonb not null default '[]'::jsonb,
  candidates jsonb not null default '[]'::jsonb,
  export_preview jsonb not null default '{}'::jsonb,
  internal_warnings text[] not null default '{}'::text[],
  status public.content_status not null default 'draft',
  visibility public.content_visibility not null default 'private_admin',
  reviewed_by_human boolean not null default false,
  ...
  constraint weekly_digest_date_order check (week_end >= week_start)
);
...
create unique index teams_provider_external_unique_idx
  on public.teams (source_provider_id, api_team_id)
  where source_provider_id is not null and api_team_id is not null;
...
create unique index players_provider_external_unique_idx
  on public.players (source_provider_id, api_player_id)
  where source_provider_id is not null and api_player_id is not null;
...
create unique index matches_provider_external_unique_idx
  on public.matches (source_provider_id, api_match_id)
  where source_provider_id is not null and api_match_id is not null;
create unique index matches_fallback_identity_unique_idx
  on public.matches (competition_id, season, home_team_id, away_team_id, kickoff_at)
  where api_match_id is null and kickoff_at is not null;
...
create unique index match_events_provider_external_unique_idx
  on public.match_events (source_provider_id, api_event_id)
  where source_provider_id is not null and api_event_id is not null;
...
create unique index player_season_stats_without_team_unique_idx
  on public.player_season_stats (competition_id, season, player_id, source_provider_id)
...
create index apify_usage_provider_idx on public.apify_usage_logs (provider_id, started_at desc);
...
create unique index user_saved_items_competition_unique_idx on public.user_saved_items (user_id, competition_id) where competition_id is not null;
create unique index user_saved_items_team_unique_idx on public.user_saved_items (user_id, team_id) where team_id is not null;
create unique index user_saved_items_player_unique_idx on public.user_saved_items (user_id, player_id) where player_id is not null;
create unique index user_saved_items_match_unique_idx on public.user_saved_items (user_id, match_id) where match_id is not null;
create unique index user_saved_items_article_unique_idx on public.user_saved_items (user_id, article_id) where article_id is not null;
create unique index user_saved_items_video_unique_idx on public.user_saved_items (user_id, video_radar_item_id) where video_radar_item_id is not null;
...
create index historical_echoes_status_visibility_idx on public.historical_echoes (status, visibility, published_at desc);
create index historical_echoes_story_idx on public.historical_echoes (related_story_id);
create index historical_echoes_match_idx on public.historical_echoes (modern_match_id);
create index historical_echoes_approved_by_idx on public.historical_echoes (approved_by);
create index daily_radar_runs_status_idx on public.daily_radar_runs (status, radar_date desc);
create index daily_radar_runs_approved_by_idx on public.daily_radar_runs (approved_by);
create index weekly_digests_status_idx on public.weekly_digests (status, week_start desc);
create index weekly_digests_approved_by_idx on public.weekly_digests (approved_by);
...
create index api_usage_provider_date_idx on public.api_usage_logs (provider_id, date desc);
...
begin
  foreach table_name in array array[
    ...
    'story_library', 'story_matches', 'historical_echoes', 'daily_radar_runs',
    'weekly_digests', 'competition_highlight_sources', 'highlight_links',
